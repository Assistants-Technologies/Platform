...
import { UiNode, UiNodeTypeEnum, UiNodeInputAttributes, UiContainer, UiNodeInputAttributesTypeEnum } from "@ory/client"
...
export default function UiNodeInput({ node }: InputProps) {
    ...
    const isButton = ([UiNodeInputAttributesTypeEnum.Button, UiNodeInputAttributesTypeEnum.Submit] as UiNodeInputAttributesTypeEnum[]).includes(node.attributes.type)
            ...
            }
            {
                ...
                (
                    <button
                        ...
                        type={node.attributes.type as "button" | "submit" | "reset" | undefined}
                        disabled={node.attributes.disabled ?? undefined}
                        className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm 
                                focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        onClick={node.attributes.onclickTrigger ? () => {
                            const fn = (window as any)[(node.attributes as any).onclickTrigger]
                            fn()
                        } : undefined}
                    >
                ...
                (
                    <node.attributes.node_type
                        ...
                        onChange={node.attributes.type === UiNodeInputAttributesTypeEnum.Checkbox ? (e) => {
