...
export type CreateBrowserLoginFlowResult =
  | { ok: true; status: 200; data: LoginFlow; error?: never; redirectTo?: never }
  | { ok: false; status: 303; redirectTo: string; data?: never; error?: never }
  | { ok: false; status: Exclude<number, 200 | 303>; error: GenericError; data?: never; redirectTo?: never }

export async function createBrowserLoginFlow(): Promise<CreateBrowserLoginFlowResult> {
  try {
