...
export type GetLoginFlowResult =
  ...
  | { ok: false; status: Exclude<number, 200 | 403 | 404 | 410>; error: GenericError; data?: never }

export async function getLoginFlow(flowId: string): Promise<GetLoginFlowResult> {
  try {
    ...
    switch (status) {
      ...
      default:
        return { ok: false, status: status as Exclude<number, 200 | 403 | 404 | 410>, error: data as GenericError }
    }
  }
}